...
layui.define('layer', function(exports){
  ...
  ,Form = function(){
    this.config = {
      verify: {
    	...
    	,uPattern: [
          /^[a-zA-Z0-9_-]{4,16}$/
          ,'登录账户请填写4到16位（字母、数字、下划线或减号）'
        ]
    	,pPattern: [
    	  /^[a-zA-Z0-9_-]{6,16}$/
          ,'密码请填写6到16位（字母、数字、下划线或减号）'
        ]
        ,email: [
  ...
  //验证规则设定
  Form.prototype.verify2 = function(settings){
	  var that = this;
	  $.extend(true, that.config.verify2, settings);
	  return that;
  };
  ...
  //表单提交校验
  var submit = function(){
    var button = $(this), verify = form.config.verify, verify2=form.config.verify2,stop = null
    ,DANGER = 'layui-form-danger', field = {} ,elem = button.parents(ELEM)
    
    ,verifyElem = elem.find('*[lay-verify]') //获取需要校验的元素
    ,verifyElem2 = elem.find('*[lay-verify2]') //获取需要校验的元素
    ...
    //开始校验2
    layui.each(verifyElem2, function(_, item){
    	var othis = $(this), ver = othis.attr('lay-verify2').split('|');
    	var tips = '', value = othis.val();
    	othis.removeClass(DANGER);
    	layui.each(ver, function(_, thisVer){
    		var isFn = typeof verify2[thisVer] === 'function';
    		if(verify2[thisVer] && (isFn ? tips = verify2[thisVer](value, item) : !verify2[thisVer][0].test(value)) ){
    			layer.msg(tips || verify2[thisVer][1], {
    				icon: 5
    				,shift: 6
    			});
    			//非移动设备自动定位焦点
