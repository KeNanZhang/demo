layui.define([ 'layer','hand','element' ], function(exports) { // 提示：组件也可以依赖其它组件，如：layui.define('layer',
	...
	var fun = {
			 ...
			 * {
			 * 
			 * mode:  1.加载数据线(将容器class设为ztree) 2.在文本框中加载当前监管单位  默认是在tab中加载树
			 * dataLineID: 数据线ID
			 ...
			 * }
			 */
			mode:null,dataLineID:null,dataLineType:null,url:null,type:null,name:null,treeModel:null,async:null,
			asyncUrl:null,dataFilter:null,filter:null,addHoverDom:null,removeHoverDom:null,
				...
				if(conf.mode == 1){
					...
					hand.ajax({
	    				...
	    				success:function(data,msg){
	    						...
	    						var line = data[key]; 
	    						if(line.length == 0){
	    							layer.msg('没有找到监管机构', {time: 3000, icon:5});
	    							return;
	    						}
	    						var setting = {
      					    			data: {
					...
					hand.ajax({
	            		url:"/supervisor/filterPermission/selectByID",
	            		type:"get",
	            		success:function(data,msg) {
	            			if(data)$("input[name=superviseName]").val(data.name);
	            		}
					});
				}else{
					var async = false;
